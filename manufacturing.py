...
closeRate = float(sys.argv[2])
rejectRate = float(sys.argv[3])
maxRawBin = int(sys.argv[4])
restockUnits = int(sys.argv[5])
restockTime = int(sys.argv[6])
fixTime = float(sys.argv[7])
workTime = float(sys.argv[8])
workstationsNumber = int(sys.argv[9])

workstationsRates = [float(rate) for rate in sys.argv[-1].split()]
...
class Product(object):
    ...
    def stopProduction(self, time: float) -> None:
        ...
        self._totalStatus[self._status] += 1
        if self._factory._day in self._totalStatusDay:
            self._totalStatusDay[self._factory._day][self._status] +=1
        self._statusTimes[self._status] = self._factory._env.now
        self._endClock = time
...
def main() -> None:
    ...
    for i in range(1,days+1):
        ...
        for wk in factory._workstations:
            ...
            wkProudctionDay = {
                str(wk._id) : {
                    "productsProcessed": len(wk._totalProductsDays[day]),
    ...
    #print(obj)
    #print(obj)
    #delete_collection(db.collection("sim"))
    now = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    ###
    doc_ref = db.collection("runs").document(now)
    doc_ref.set({ "Status": "1" });
    ###
    doc_ref = db.collection(now)
    docs = doc_ref.stream()
    docsNum = 0
    for doc in docs:
        docsNum += 1
        #doc.reference.delete()
    doc_ref = db.collection(now).document("TotalProd")
    del obj["sim"]["production"]["daily"]
    doc_ref.set(obj["sim"])
    print(len(completeDaysProduction))
    for day in completeDaysProduction:
        doc_ref = db.collection(now).document(day[1:].replace("/", "-"))
        dayData = completeDaysProduction[day]
        print("-------------------")
        #print(completeDaysProduction[day])
        doc_ref.set(dayData)
    ####
